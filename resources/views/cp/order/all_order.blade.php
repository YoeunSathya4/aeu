...
@section ('section-js')
	...
	function updateDelivery(id){
     	...
     	$.ajax({
	        url: "{{ route($route.'.update-delivery') }}",
	        method: 'POST',
	        data: {id:id, active:active },
	        success: function( response ) {
	            if ( response.status === 'success' ) {
		...
		<tbody>
			...
			@foreach ($data as $row)
				<tr>
					...
					<td>
						<div class="checkbox-toggle">
					        <input onclick="updateDelivery({{ $row->id }})" type="checkbox" id="delivery-{{ $row->id }}" @if ($row->is_delivery == 1) checked data-value="1" @else data-value="0" @endif >
					        <label for="delivery-{{ $row->id }}"></label>
				        </div>
					</td>
