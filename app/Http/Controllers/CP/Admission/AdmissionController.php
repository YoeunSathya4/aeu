...
namespace App\Http\Controllers\CP\Admission;
...
use App\Model\Admission\Admission as Model;
...
class AdmissionController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        $data = array(
                    ...
                    'slug'          =>   GenerateSlug::generateSlug('admissions', $request->input('en_title')),
        ...
        if($request->hasFile('image')) {
            ...
            Image::make($image->getRealPath())->resize(262, 205)->save(public_path('uploads/admission/image/'.$imagename));
            $data['image']= 'public/uploads/admission/image/'.$imagename;
        }else{
            $data['image']='';  
        }
        ...
        $tracking->description = 'Create new admission name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function update(Request $request){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update admission id:'.$id.'and name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function trash($id){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Delete admission name:'.$tracking_data->en_title;
        $tracking->save();
        Session::flash('msg', 'Data has been delete!' );
        return response()->json([
            'status' => 'success',
            'msg' => 'Admission has been deleted'
        ]);
    }
    ...
    function updateStatus(Request $request){
      ...
      if($request->input('active') == 1){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update admission publish name:'.$tracking_data->en_title;
        $tracking->save();
    }else{
        $tracking = new Tracking();
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update admission unpublish name:'.$tracking_data->en_title;
        $tracking->save();
    }
      return response()->json([
          'status' => 'success',
