...
namespace App\Http\Controllers\CP\Publication;
...
use App\Model\Publication\Publication as Model;
...
class PublicationController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        $data = array(
                    ...
                    'slug'      =>   GenerateSlug::generateSlug('publications', $request->input('en_title')),
        ...
        if($request->hasFile('image')) {
            ...
            Image::make($image->getRealPath())->resize(262, 205)->save(public_path('uploads/publication/image/'.$imagename));
            $data['image']= 'public/uploads/publication/image/'.$imagename;
        }else{
            $data['image']='';  
        }
        ...
        $tracking->description = 'Create new publication name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function update(Request $request){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update publication id:'.$id.'and name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function trash($id){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Delete publication name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    function updateStatus(Request $request){
      ...
      if($request->input('active') == 1){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update publication publish name:'.$tracking_data->en_title;
        $tracking->save();
    }else{
        $tracking = new Tracking();
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update publication unpublish name:'.$tracking_data->en_title;
        $tracking->save();
    }
      return response()->json([
          'status' => 'success',
