...
namespace App\Http\Controllers\CP\Faculty;
...
use App\Model\Faculty\Main as Model;
...
class FacultyController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        if($request->hasFile('image')) {
            ...
            Image::make($image->getRealPath())->resize(262, 205)->save(public_path('uploads/faculty/image/'.$imagename));
            $data['image']= 'public/uploads/faculty/image/'.$imagename;
        }else{
            $data['image']='';  
        }
    ...
    public function update(Request $request){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update faculty id:'.$id.'and name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function trash($id){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Delete faculty name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    function updateStatus(Request $request){
      ...
      if($request->input('active') == 1){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update faculty publish name:'.$tracking_data->en_title;
        $tracking->save();
    }else{
        $tracking = new Tracking();
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update faculty unpublish name:'.$tracking_data->en_title;
        $tracking->save();
    }
      return response()->json([
          'status' => 'success',
