...
namespace App\Http\Controllers\CP\Partner;
...
use App\Model\Partner\Main as Model;
...
class PartnerController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        if($request->hasFile('image')) {
            ...
            Image::make($image->getRealPath())->resize(300, 300)->save(public_path('uploads/partner/image/'.$imagename));
            $data['image']= 'public/uploads/partner/image/'.$imagename;
        }else{
            $data['image']='';
        }
    ...
    public function update(Request $request){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update partner name:'.$tracking_data->name;
        $tracking->save();
    ...
    public function trash($id){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Delete partner name:'.$tracking_data->name;
        $tracking->save();
    ...
    function updateStatus(Request $request){
      ...
      if($request->input('active') == 1){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update partner publish name:'.$tracking_data->name;
        $tracking->save();
    }else{
        $tracking = new Tracking();
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update partner unpublish name:'.$tracking_data->name;
        $tracking->save();
    }
