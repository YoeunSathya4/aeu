...
namespace App\Http\Controllers\CP\Alumnies;
...
use App\Model\Alumnies\Alumnies as Model;
...
class AlumniesController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        $data = array(
                    ...
                    'slug'      =>   GenerateSlug::generateSlug('alumnies', $request->input('en_title')),
        ...
        if($request->hasFile('image')) {
            ...
            Image::make($image->getRealPath())->resize(262, 205)->save(public_path('uploads/alumnies/image/'.$imagename));
            $data['image']= 'public/uploads/alumnies/image/'.$imagename;
        }else{
            $data['image']='';  
        }
        ...
        $tracking->description = 'Create new alumnies name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function update(Request $request){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update alumnies id:'.$id.'and name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function trash($id){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Delete alumnies name:'.$tracking_data->en_title;
        $tracking->save();
        Session::flash('msg', 'Data has been delete!' );
        return response()->json([
            'status' => 'success',
            'msg' => 'Alumnies has been deleted'
        ]);
    }
    ...
    function updateStatus(Request $request){
      ...
      if($request->input('active') == 1){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update alumnies publish name:'.$tracking_data->en_title;
        $tracking->save();
    }else{
        $tracking = new Tracking();
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update alumnies unpublish name:'.$tracking_data->en_title;
        $tracking->save();
    }
      return response()->json([
          'status' => 'success',
