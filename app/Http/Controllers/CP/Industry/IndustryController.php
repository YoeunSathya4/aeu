...
namespace App\Http\Controllers\CP\Industry;
...
use App\Model\Industry\Industry as Model;
...
class IndustryController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        $data = array(
                    ...
                    'slug'      =>   GenerateSlug::generateSlug('industries', $request->input('en_title')),
        ...
        if($request->hasFile('image')) {
            ...
            Image::make($image->getRealPath())->resize(262, 205)->save(public_path('uploads/industry/image/'.$imagename));
            $data['image']= 'public/uploads/industry/image/'.$imagename;
        }else{
            $data['image']='';  
        }
        ...
        $tracking->description = 'Create new industry name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function update(Request $request){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update industry id:'.$id.'and name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function trash($id){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Delete industry name:'.$tracking_data->en_title;
        $tracking->save();
        Session::flash('msg', 'Data has been delete!' );
        return response()->json([
            'status' => 'success',
            'msg' => 'Industry has been deleted'
        ]);
    }
    ...
    function updateStatus(Request $request){
      ...
      if($request->input('active') == 1){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update industry publish name:'.$tracking_data->en_title;
        $tracking->save();
    }else{
        $tracking = new Tracking();
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update industry unpublish name:'.$tracking_data->en_title;
        $tracking->save();
    }
      return response()->json([
          'status' => 'success',
