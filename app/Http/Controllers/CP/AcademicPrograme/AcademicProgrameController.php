...
namespace App\Http\Controllers\CP\AcademicPrograme;
...
use App\Model\AcademicProgram\AcademicProgram as Model;
use App\Model\User\Tracking;
use Image;


class AcademicProgrameController extends Controller
{
    protected $route; 
    public function __construct(){
        $this->route = "cp.academic-programe";
    }
    ...
    public function store(Request $request) {
        ...
        $data = array(
                    ...
                    'slug'      =>   GenerateSlug::generateSlug('academic_programs', $request->input('en_title')),
        ...
        if($request->hasFile('image')) {
            ...
            Image::make($image->getRealPath())->resize(262, 205)->save(public_path('uploads/academic_programe/image/'.$imagename));
            $data['image']= 'public/uploads/academic_programe/image/'.$imagename;
        }else{
            $data['image']='';  
        }
        ...
        $tracking->description = 'Create new academic programe name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function update(Request $request){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update academic programe id:'.$id.'and name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    public function trash($id){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Delete academic programe name:'.$tracking_data->en_title;
        $tracking->save();
    ...
    function updateStatus(Request $request){
      ...
      if($request->input('active') == 1){
        ...
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update academic programe publish name:'.$tracking_data->en_title;
        $tracking->save();
    }else{
        $tracking = new Tracking();
        $tracking->user_id = $user_id;
        $tracking->created_at = $now;
        $tracking->description = 'Update academic programe unpublish name:'.$tracking_data->en_title;
        $tracking->save();
    }
      return response()->json([
          'status' => 'success',
